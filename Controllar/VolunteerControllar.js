...
const createRecord = async (req, res) => {
    try {
        ...
        let { donation, sirName, firstName, lastName, dob, panNo, email, mobile, address, country, state, city, pinCode, citizenship, helpMessage, money ,intersted } = req.body;
        if (!donation || !sirName || !firstName || !lastName || !dob || !panNo || !email || !mobile || !address || !country || !state || !city || !pinCode || !citizenship || !helpMessage || !money || !intersted) {
            return res.status(403).json({
                success: false,
                mess: "Fill all required fields"
            });
        } else {
            const data = new volunteer({ donation, sirName, firstName, lastName, dob, panNo, email, mobile, address, country, state, city, pinCode, citizenship, helpMessage, money ,intersted });
            await data.save();
